...
function drawBottomBar(){
  ...
  drawCaptureIconButton(SAFE + 710, y0+26, 110, 70);
}
...
async function onRemoteCapture(row) {
  ...
  // bake old top into wall
  if (topSticker) bakeTopStickerToWall();
  ...
  topSticker = {
    ...
    dx: 0,
    dy: 0,
    editable: (row.user_id === USER_ID) // ✅ only owner can edit
  };
}
...
async function loadSnapshotIfAny(){
  ...
  // 把快照画进 wallBase（右侧 local coords）
  wallBase.clear();
